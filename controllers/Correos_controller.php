...
class Correos_controller extends Controller {
    ...
    function CorreoMisClientes($correo_remitente, $correo_destinatario, $nombre_detinatario, $cuerpo) {
        $mail = new PHPMailer;
        $mail->isSMTP();
        $mail->Host = 'mail.gingercat.ml';
        $mail->SMTPAuth = true; //
        $mail->Username = 'crm@gingercat.ml';
        $mail->Password = 'kassandra@2015';
        $mail->SMTPSecure = 'ssl';
        $mail->Port = 465;
        $mail->setFrom('crm@gingercat.ml', 'CRM AMDDIGITAL');
